...
test_simulation_data_X = pd.concat([test_X[TEST_SIZE:TEST_SIZE*2], new_fraud_data[FRAUD_FEATURES].head(TEST_SIZE*4)], ignore_index=True)
test_simulation_data_y = pd.concat([test_y[TEST_SIZE:TEST_SIZE*2], new_fraud_data['fraud_bool'].head(TEST_SIZE*4)], ignore_index=True)
